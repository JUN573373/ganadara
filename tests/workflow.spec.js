...
test('수집 후 이동하고 브라우저를 다시 열어 수정받은 엑셀로 배송문자를 만든다', async ({ page }) => {
  ...
  await page.locator('.col-shipping').fill('수정한샵');
  const popupPromise1 = page.waitForEvent('popup');
  await page.locator('#view-original').click();
  const popup1 = await popupPromise1;
  await expect(popup1.locator('.original-list')).toContainText('테스트샵');
  await expect(popup1.locator('.original-list')).not.toContainText('수정한샵');
  await popup1.close();
  await page.reload();
  ...
  expect(JSON.parse(stored).originalData[0].rows[0]['배송지']).toBe('테스트샵');
  expect(JSON.parse(stored).rows[0]['배송지']).toBe('수정한샵');
  ...
  await expect(popup2.locator('.original-list')).toContainText('테스트샵');
  await popup2.close();
  ...
  await expect(page.locator('#sms-text')).toHaveValue(/10시 30분 수정한샵/);
